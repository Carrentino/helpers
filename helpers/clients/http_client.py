...
from helpers.api.middleware.trace_id.constants import DEFAULT_TRACE_ID_HEADER_NAME
from helpers.contextvars import TRACE_ID
from helpers.errors import ServerError
from helpers.json import dump_json
...
async def _trace_id_header_event_hook(
    ...
    if request.headers.get(DEFAULT_TRACE_ID_HEADER_NAME) is None:
        request.headers[DEFAULT_TRACE_ID_HEADER_NAME] = trace_id
...
class BaseApiClient(AsyncClient):
    ...
    _headers: ClassVar[dict[str, str]] = {'Content-Type': 'application/json'}  # type: ignore
    _timeout: int = 5.0  # type: ignore
